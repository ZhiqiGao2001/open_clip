...
class RegressionLoss(nn.Module):
    ...
    def forward(self, image_features, text_features, logit_scale, output_dict=False):
        ...
        # loss is determined by cosine similarity
        total_loss = (
                F.mse_loss(F.normalize(logits_per_image, p=2, dim=1), F.normalize(logits_per_text, p=2, dim=1))
        )
        return {"angle loss": total_loss} if output_dict else total_loss
